...
app = FastAPI(
    title="Mordzix AI",
    version="5.0.0",
    description="Zaawansowany system AI z pamiêci¹, uczeniem i pe³n¹ automatyzacj¹",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
except ImportError:  # pragma: no cover - fallback dla œrodowisk bez uvicorn
    uvicorn = None
...
# Lista endpointów wymagaj¹cych rêcznej akceptacji (spójna z frontendem)
MANUAL_TOOL_ENDPOINTS: List[Dict[str, str]] = [
    {
        "name": "code_write",
        "endpoint": "POST /api/code/write",
        "reason": "Zapisuje pliki w repozytorium i wymaga œwiadomego potwierdzenia."
    },
    {
        "name": "code_deps_install",
        "endpoint": "POST /api/code/deps/install",
        "reason": "Instaluje zale¿noœci i modyfikuje œrodowisko uruchomieniowe."
    },
    {
    ...
    },
    {
        ...
        "reason": "Wysy³a polecenia git zmieniaj¹ce historiê repozytorium."
    },
    {
        "name": "code_init",
        "endpoint": "POST /api/code/init",
        "reason": "Tworzy now¹ strukturê projektu na dysku i mo¿e nadpisaæ pliki."
    },
]
...
# Czy logowaæ podczas importu (przy np. narzêdziach CLI ustawiamy flagê by wyciszyæ)
...
def _load_fast_path_handlers() -> Dict[str, Any]:
    """Zwróæ listê nazw handlerów fast path (bezpoœrednie regexy)."""
    ...
    except Exception as exc:  # pragma: no cover - œrodowiska bez modu³u
...
def _load_tool_registry() -> Dict[str, Any]:
    """Zwróæ listê narzêdzi routera LLM."""
...
def get_automation_summary(refresh: bool = False) -> Dict[str, Any]:
    """Pobierz (opcjonalnie odœwie¿) cache z podsumowaniem automatyzacji."""
    ...
    if refresh or not _AUTOMATION_SUMMARY_CACHE:
        ...
        # Do³¹cz timestamp do cache (mo¿e byæ potrzebny przy monitoringu)
        _AUTOMATION_SUMMARY_CACHE["generated_at"] = _AUTOMATION_SUMMARY_TS

    return _AUTOMATION_SUMMARY_CACHE

# Prometheus middleware korzysta z core.metrics (jeœli dostêpne)
...
# 25. NLP (zaawansowana analiza jêzykowa)
try:
    import nlp_endpoint
...
# 29. INTERNAL UI (interfejs wewnêtrzny)
try:
    import internal_ui
...
if not _SUPPRESS_IMPORT_LOGS:
    ...
    print("? Self-Reflection Engine (5 poziomów g³êbokoœci)")
...
async def automation_status():
    """Podsumowanie automatycznych narzêdzi i fast path."""
...
# POPRAWKA: BASE_DIR z config.py mo¿e byæ b³êdne gdy importuje z core/
# U¿ywamy Path(__file__).parent.parent.resolve() aby dostaæ /home/ubuntu/mrd
from pathlib import Path as _Path
_APP_ROOT = _Path(__file__).parent.parent.resolve()
FRONTEND_DIST = _APP_ROOT / "frontend" / "dist" / "mordzix-ai"

# Serwowanie statycznych plików z Angular dist/ (tylko jeœli istniej¹)
...
`n# `n# WebUI - MUSI BYÆ PRZED catch-all!`n# `nwebui_dir = BASE_DIR.parent / "webui"`nif webui_dir.exists():`n    app.mount("/webui", StaticFiles(directory=str(webui_dir)), name="webui")`n    print(" WebUI mounted at /webui/")`n    `n    @app.get("/webui/", include_in_schema=False)`n    async def webui_index():`n        return FileResponse(str(webui_dir / "index.html"))`nelse:`n    print(f" WebUI not found: {webui_dir}")`n`n# Catch-all dla Angular routing (musi byæ na koñcu!)
@app.get("/{full_path:path}", response_class=HTMLResponse, include_in_schema=False)
async def angular_catch_all(full_path: str):
    """Przekieruj wszystkie nieznane œcie¿ki do Angular SPA (dla routingu)"""
    # Ignoruj œcie¿ki API
    if full_path.startswith("api/") or full_path.startswith("health"):
        raise HTTPException(status_code=404, detail="API endpoint not found")
    
    # Zwróæ Angular index.html (SPA obs³u¿y routing)
    angular_index = FRONTEND_DIST / "index.html"
    if angular_index.exists():
        return HTMLResponse(content=angular_index.read_text(encoding="utf-8"))
...
webui_dir = BASE_DIR.parent / "webui"  # Katalog g³ówny projektu
if webui_dir.exists():
...
async def startup_event():
    ...
    # Inicjalizacja bazy danych i pamiêci
    try:
        from core.memory import _init_db, load_ltm_to_memory
        _init_db()
        load_ltm_to_memory()
        print("[OK] Pamiêæ LTM za³adowana")
    except Exception as e:
        print(f"[WARN] B³¹d inicjalizacji pamiêci: {e}")
...
if __name__ == "__main__":
    ...
    if uvicorn is None:
        raise RuntimeError("Uvicorn nie jest zainstalowany. Uruchom 'pip install uvicorn' w œrodowisku aplikacji.")
