...
class AdvancedCognitiveEngine:
    ...
    async def _should_use_web_research(self, user_message: str, memory_context: List[Dict[str, Any]]) -> bool:
        """Sprawdź czy potrzebny jest web research"""
        # Słowa kluczowe sugerujące potrzebę aktualnych informacji
        current_info_keywords = [
            "najnowsze", "aktualne", "dzisiaj", "teraz", "ostatnie", "bieżące",
            "wiadomości", "news", "co się dzieje", "wydarzenia", "informacje",
            "pogoda", "kurs", "cena", "notowania", "wyniki"
        ]
        
        message_lower = user_message.lower()
        
        # Sprawdź czy zapytanie zawiera słowa kluczowe
        has_keywords = any(keyword in message_lower for keyword in current_info_keywords)
        
        # Sprawdź czy w pamięci jest wystarczająco dużo informacji
        has_sufficient_memory = len(memory_context) >= 3
        
        # Użyj web research jeśli:
        # 1. Są słowa kluczowe sugerujące aktualność
        # 2. Brak wystarczających informacji w pamięci
        should_research = has_keywords or not has_sufficient_memory
        
        if should_research:
            log_info(f"[COGNITIVE_ENGINE] Web research triggered: keywords={has_keywords}, memory={len(memory_context)} items")
    ...
    async def _generate_enhanced_response(
        ...
        if autonauka and await self._should_use_web_research(user_message, memory_context):
            try:
                log_info("[COGNITIVE_ENGINE] 🌐 Wykonuję web research...")
                web_result = await autonauka(user_message, topk=5, deep_research=False, user_id="system")
